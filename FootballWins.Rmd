...
ANSWER HERE: Based on the bullet points above, the model appears to be fairly useful, even though the RSE is just over 2. Given how many games are in a football season, the model deviating by 2 seems like a significant difference, but the multiple r-squared indicates a good fit, and the F-statistic of 15.44 is indicative of a fairly accurate model.  
...
ANSWER HERE: All three factors have varrying degrees of impact on a team's win total, but points scored is unsurprisingly the most impact -- with a correlation coefficient of around 0.8. This isn't surprising in the slightest, because football is a sport in which the team that scores more points than the other in a given game, wins said game. Of the three measures, takeaways has the smallest impact on wins, which is comparable to real life; Maryland finished its regular season tied for 15th in the country with 21 takeaways, yet the Terps finished 4-8 and didn't win a game after September.       

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

This model could help a reporter visualize how complete a team may be playing, and why certaiin positive aspects don't necessarily lead to wins. Using the Maryland example, the Terps were one of the better teams in the country this year in regard to creating turnovers -- specifically interceptions. But they sruggled mightly with turning those takeaways into points, which if why their positive turnover margin didn't manifest in positive results. Generally speaking, I think a reporter could you this model as story inspiration, to explore why the Terp struggled to put up points off of takeaways, since turonvers only truely matter if a team can consistently turn them into scores.  
